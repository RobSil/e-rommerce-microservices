...
public class CartController {
    ...
    @GetMapping
    public ResponseEntity<DetailedCartDto> getByCurrentUser(UserAuthenticationToken user) {
        return new ResponseEntity<>(cartFacadeService.findDetailedCartByUser(user), HttpStatus.OK);
    }

    @DeleteMapping("/all")
    public ResponseEntity<Void> deleteAllByCurrentUser(UserAuthenticationToken user) {
